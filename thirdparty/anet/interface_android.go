...
// zoneCache and zoneCacheX were originally linked (via //go:linkname) to
// the standard library's and x/net's own private IPv6 zone caches, so this
// package's cache refresh would keep them in sync too. That linkname
// target's layout no longer matches this Go toolchain's internal net
// package (breaks the build with "invalid reference to net.zoneCache"),
// and upstream anet has no newer release fixing it, so this local
// replace-directive fork (see the repo's go.mod) drops the link and keeps
// these as plain package-local caches instead. Functionally harmless here:
// libp2p only calls Interfaces()/InterfaceAddrs() for interface listing,
// never anything relying on net's own zone cache being warmed as a
// side effect.
